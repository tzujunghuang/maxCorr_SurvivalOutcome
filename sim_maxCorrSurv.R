...
cr = '10%'
# Methods to run
meth_vecs = c('BONF_COX', 'Naive_OSE', 'BONF_OSE', 'OOSE', 'SOSE')

# List of (n,p) values to run simulation
#   np_list = list(c(500, 1e6))
np_list = list(c(500, 1e5), c(500, 1e4), c(500, 1000), c(500, 100))
...
sim = data.frame(method=NA, model=NA, n=NA, p=NA, rho=NA, quar_trunc=NA, elln=NA, Rej=NA, est=NA, se=NA)
	  ...
	  for (meth in meth_vecs) {
	    if (meth=='SOSE') {
	      ...
	      SOSE_est = do.call(rbind, lapply(1:length(elln_part), function(d_indx){
	        ...
	        SOSE_est = obj0$Stab_onestep_est(all_obs=1:n, chunk_size, elln, est_index=est_index_val, 
	      ...
	      out$SOSE = SOSE_est
		    ...
		    print(meth)
		    OOSE_est = obj0$Oracle_onestep_est(alpha=alpha_val, quar_trunc=quar, idx_taken=1)
		    out$OOSE = c( OOSE_est$rej, OOSE_est$est, OOSE_est$se )
		          
		  } else if (meth=='Naive_OSE') {
		    
		    print(meth)
		    Naive_OSE_est = obj0$Naive_onestep_est(alpha=alpha_val, quar_trunc=quar, num_top=1)
		    ...
		    print(meth)
		    BONF_OSE_est = obj0$Bonf_onestep_ests(alpha=alpha_val, quar_trunc=quar)
	  ...
	  sim = rbind(sim, do.call(rbind,lapply(meth_vecs, function(meth) {
	    if ( meth=='SOSE' ) {
	      result = data.frame( method = rep(meth, dim_elln), model = rep(model, dim_elln),
	                           n = rep(n, dim_elln), p = rep(p, dim_elln), rho = rep(rho, dim_elln), 
	                           quar_trunc = rep(quar, dim_elln), elln = out[[meth]][,5],
	                           Rej = 1*( out[[meth]][,1]>0 | out[[meth]][,2]<0 ),
	                           est = out[[meth]][,3], se = out[[meth]][,4] )
	    
	    } else if ( meth=='BONF_COX' ) {
	      result = data.frame( method = meth, model = model, n = n, p = p, rho = rho, quar_trunc = quar, elln = NA, 
	                           Rej = 1*(out[[meth]][1]>0), est = NA, se = NA )
	    } else {
	      result = data.frame( method = meth, model = model, n = n, p = p, rho = rho, quar_trunc = quar, elln = NA, 
	                           Rej = 1*(out[[meth]][1]>0), est = out[[meth]][2], se = out[[meth]][3] )
	    }
	    return ( result ) })))
  }
}
