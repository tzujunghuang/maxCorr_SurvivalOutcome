...
elln_quar_list = expand.grid(elln_part, quars)
      ...
      for (np in np_list) {
        ...
        if (meth=='SOSE') {
          ...
          results = do.call(rbind, lapply(1:dim(elln_quar_list)[1], function(row_idx){
            ...
            condition1 = ( sim$method==meth & sim$model==model & sim$n==n & sim$p==p & sim$rho==rho_val &
                           sim$quar_trunc==quar & sim$elln==elln )

            est = sim[condition1, 'est']; se =  sim[condition1, 'se']
            rej =  sim[condition1, 'Rej']
            ret = data.frame(model=model, rho=rho_val, meth=meth, n=n, p=p, quar=quar, elln=elln, 
                             est=est, se=se, rej=rej, replic_num=r_idx)
            return( ret ) }))
        } else { 
          #print(meth)
          results = do.call(rbind, lapply(1:length(quars), function(idx){
            ...
            est = sim[condition1, 'est']; se = sim[condition1, 'se']
            rej =  sim[condition1, 'Rej']
            ret = data.frame(model=model, rho=rho_val, meth=meth, n=n, p=p, quar=quar, elln=NA, 
                             est=est, se=se, rej=rej, replic_num=r_idx)
            return( ret ) })) }    
...
save(final, file=paste0('results_maxCorrSurv_cr', cr_num, '_', est_index_val, '.Rdata'))
