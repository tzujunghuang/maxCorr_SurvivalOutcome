  ...
  if (num_portions == 0) {
    ...
    results <- do.call(cbind, lapply(0:num_portions, function(i) {
      ...
      if (i < num_portions) {
        n0 = dim(mat)[1];
        sub_mat = mat[,(1+i*block_size):((i+1)*block_size)]
        sub_mean_mat = matrix(rep(colMeans_mat[(1+i*block_size):((i+1)*block_size)], n0), byrow=TRUE, nrow=n0)
...
colIF_ipw <- function(mat, colMeans_mat, colVars_mat, colCovs_mat, m, central_Y_Ghat=Y_Ghat-mean_YGhat, block_size=1e4) {
  ...
  if (num_portions == 0) {
    ...
    results <- do.call(cbind, lapply(0:num_portions, function(i) {
      if (i < num_portions) {
        n0 = dim(mat)[1] 
        sub_mat = mat[,(1+i*block_size):((i+1)*block_size)]
        sub_mu_selectU_0 = matrix(rep(colMeans_mat[(1+i*block_size):((i+1)*block_size)], n0), byrow=TRUE, nrow=n0)
        sub_var_selectU_0 = matrix(rep(colVars_mat[(1+i*block_size):((i+1)*block_size)], n0), byrow=TRUE, nrow=n0)
        sub_cov_selectU0_Y = matrix(rep(colCovs_mat[(1+i*block_size):((i+1)*block_size)], n0), byrow=TRUE, nrow=n0)
        ...
        result = matrix(rep(m[(1+i*block_size):((i+1)*block_size)], n0), byrow=TRUE, nrow=n0)*(a-b)
        ...
        sub_cov_selectU0_Y = matrix(rep(colCovs_mat[(1+i*block_size):dim(mat)[2]], n0), byrow=TRUE, nrow=n0)
  ...
  if (num_portions == 0) {
    ...
    results <- do.call(cbind, lapply(0:num_portions, function(i) {
      if (i < num_portions) {
        ...
        sub_mu_selectU_0 = matrix(rep(colMeans_mat[(1+i*block_size):((i+1)*block_size)], n0), byrow=TRUE, nrow=n0)
        sub_var_selectU_0 = matrix(rep(colVars_mat[(1+i*block_size):((i+1)*block_size)], n0), byrow=TRUE, nrow=n0)
        result = ( matrix(rep(m[(1+i*block_size):((i+1)*block_size)], n0), byrow=TRUE, nrow=n0)
...
colStandardization <- function(mat, colSD, block_size=1e4) {
  ...
  if (num_portions == 0) {
    n0 = dim(mat)[1]
    results = as.matrix((mat - matrix(rep(my_colMeans(mat), n0), nrow=n0, byrow=TRUE))/matrix(rep(colSD, n0), nrow=n0, byrow=TRUE))
  } else {
    results <- do.call(cbind, lapply(0:num_portions, function(i) {
                  if (i < num_portions) {
                      sub_mat = mat[,(1+i*block_size):((i+1)*block_size)]
                  ...
                  nn = dim(sub_mat)[1]
                  result = (sub_mat - matrix(rep(my_colMeans(sub_mat), nn), nrow=nn, byrow=TRUE))/matrix(rep(sub_colSD, nn), nrow=nn, byrow=TRUE)
                  as.matrix(result) })) }
  return (results)
}  
