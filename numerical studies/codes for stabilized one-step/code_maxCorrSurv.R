 ...
 methods = list(
    ...
    if ( is.matrix(selectU_0) ) { 
     ...
     Ehat_pointwise_val = sapply( 1:length(obs), function(i){
       ...
       tryCatch({ mean_selectU = my_colMeans(selectU); var_selectU = colVars(selectU, sd_use = FALSE) },
           ...
           message("Original error message:"); message(paste(msg,"\n", sep = ''))
           save( input_data, file = paste('errordata_in_selectU_for', i, '_in', r, '.Rdata', sep = '') )
           stop(msg) })
  ...
  IF_star = function(m, U_index, obs_all, obs0, obs1, quar_trunc){
    ...
    error = function(msg='Error in IF_star time_comparison dimension') {
      message("Original error message:"); message(paste(msg,"\n", sep = ''))
      save( input_data, file = paste('errdata_', r, '.Rdata', sep = '') )
      stop(msg) })
    
    a = ((selectU_1 - mu_selectU_0) * (Y_Ghat_1 - mean_YGhat_0)) / var_selectU_0
    b = ((selectU_1 - mu_selectU_0)^2) * matrix(rep(cov_mat_0, length(obs1)), nrow = length(obs1), 
                                                byrow = TRUE) / (var_selectU_0)^2
  ...
  Stab_onestep_est = function(all_obs, chunk_size, elln, est_index, alpha, num_top = 1, quar_trunc){ 
    ...
    mt = rowMeans( sapply(0:(ceiling((n - elln)/chunk_size) - 1), function(i){
         ...
         curr_sigma_inv0 = 1/sd(rowMeans(IF_star(m = sgn_k0, U_index = k0, obs_all = obs_all_val, 
                                                 obs0 = obs0_val, obs1 = old_obs, quar_trunc)))
         est0 = mean( (mean(sgn_k0*cor_k0) 
                       + rowMeans(IF_star(m = sgn_k0, U_index = k0, obs_all = obs_all_val, 
  ...
  Bonf_Cox_ests = function(alpha){ # alpha is a scalar
    ...
    colnames(data_est) = c('X', 'delta', paste('V', 1:p, sep = ''))
    
    result = do.call(rbind, lapply(1:p, function(idx){
      cox_fit = coxph(Surv(X, delta) ~ data_est[,paste('V',idx, sep = '')], data_est, 
    ...
    return( list( rej = 1*(min(result[,2]) <= alpha/p),
                  ...
                  k_est = result[which.min(result[,2]), 1], 
                  k_ests = result[result[,2] <= alpha/p, 1] ) )
  }
 )
)